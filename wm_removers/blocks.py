...
from wm_removers.model_init import *
...
class SEBlock(nn.Module):
    ...
    def __init__(self, channel,reducation=16):
        ...
        self.avg_pool = nn.AdaptiveAvgPool2d(1)     # 对输入张量进行全局池化
        self.fc = nn.Sequential(            # 以下构成了全连接层
            nn.Linear(channel,channel//reducation),  # 将维度为channel的映射至channel//reducation上
            nn.ReLU(inplace=True),
            nn.Linear(channel//reducation,channel),  # 将维度为channel//reducation映射至channel上
            nn.Sigmoid())
        self.to("cuda")
    ...
    def forward(self,x):
        b,c,w,h = x.size()      # 获取x的形状信息, b为batch，c为channel
        y1 = self.avg_pool(x).view(b,c) # self.avg_pool(x)得到(b,c,1,1),.view(b,c)将其展开成(b,c)的张量y1
        # print(self.device)
        y = self.fc(y1).view(b,c,1,1) # self.fc(y1).view(b,c,1,1)得到(b,c,1,1)
        return x*y
