...
from scripts.models.mixed_attn_block import (
    AnchorProjection,
    CAB,
    CPB_MLP,
    QKVProjection,
)
from scripts.models.ops import (
    window_partition,
    window_reverse,
)

from scripts.models.swin_v1_block import Mlp
...
class AffineTransform(nn.Module):
    ...
    def __init__(self, num_heads):
        ...
        size_int = tuple(map(int, num_heads))
        size_int = size_int + (1, 1)
        # logit_scale = torch.log(10 * torch.ones((num_heads, 1, 1)))
        logit_scale = torch.log(10 * torch.ones(size_int))
...
class WindowAttention(Attention):
    ...
    def __init__(
        ...
        # num_heads = ({self.num_heads_w}, {self.num_heads_s})
        # print(num_heads)
...
class EfficientMixAttnTransformerBlock(nn.Module):
    ...
    def forward(self, x, x_size, all_table_index_mask):
        ...
        if self.args.local_connection:     # 实际上这里为false
            x = (
                x
                + self.res_scale
