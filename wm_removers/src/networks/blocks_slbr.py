...
import math
import numbers
from wm_removers.src.utils.model_init import *
...
class UpConv(nn.Module):
    def __init__(self, in_channels, out_channels, blocks, residual=True,norm=nn.BatchNorm2d,
        ...
        # 如果 dilations 参数为空，则将其设置为全 1
        if len(dilations) == 0: dilations = [1] * blocks
        # 根据 concat 参数选择不同的卷积核大小和归一化层
        if self.concat:
    ...
    def forward(self, from_up, from_down, mask=None,se=None):
        ...
        # 根据 concat 和 use_mask 参数选择不同的拼接方式
        if self.concat:
            if self.use_mask:
        ...
        x2 = None
        # 遍历 conv2 列表中的卷积层，并应用对应的归一化层
        for idx, conv in enumerate(self.conv2):
            x2 = conv(x1)
            x2 = self.bn[idx](x2)
            # 如果 se 参数不为空且为最后一个卷积层，则应用 se 操作
            if (se is not None) and (idx == len(self.conv2) - 1): # last 
                x2 = se(x2)
            # 如果 residual 参数为 True，则进行残差连接
            if self.residual:
                x2 = x2 + x1
            x2 = self.act(x2)
            x1 = x2
        # 根据 out_fuse 参数决定返回结果
        if self.out_fuse:
            return x2, xfuse
        else:
            return x2
...
class AttentionPooling(nn.Module):
    ...
    def forward(self, x):
        ...
        out = torch.sum(out, dim=(4, 5)) / (self.stride * self.stride)
        return out
...
class SMRBlock(nn.Module):
    def __init__(self, args, ins, outs, norm=nn.BatchNorm2d, act=F.relu, blocks=1, residual=True, concat=True):
        ...
        self.self_calibrated = SelfAttentionSimple(self.mode, outs, sim_metric='cos', k_center=1, project_mode='simple')
